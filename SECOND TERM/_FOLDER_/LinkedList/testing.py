...
def main():
    for line in stdin:
        ...
        while len(deck) > 1:
            discardCards = deck.pop()
            discardedCards.append(discardCards)
            cardNotDiscard = deck.pop()
            deck.append(cardNotDiscard)

        print(f"Discarded cards: {', '.join(map(str,discardedCards.to_list()))}")
