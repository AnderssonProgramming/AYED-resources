...
class LinkedList:
    ...
    def delete(self, value):  # Nuevo método para eliminar un nodo dado su valor
        ...
        if node_to_delete is not None:
            if node_to_delete.getPrev() is not None:
                node_to_delete.getPrev().setNext(node_to_delete.getNext())
            if node_to_delete.getNext() is not None:
                node_to_delete.getNext().setPrev(node_to_delete.getPrev())
            if node_to_delete == self.head:
                self.setHead(node_to_delete.getNext())
            if node_to_delete == self.tail:
                self.setTail(node_to_delete.getPrev())
            self.len -= 1

    def delete_duplicates(self):  # Nuevo método para eliminar elementos duplicados
        values = set()
        node = self.head
        while node is not None:
            if node.getValue() in values:
