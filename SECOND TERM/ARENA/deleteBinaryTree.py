...
class BinaryTree:
    ...
    def delete(self, v):
        ...
        if v < self.getRoot():
            ...
            if self.isLeaf() or (self.getLeft() is None and self.getRight() is None):  # Caso en el que el nodo es una hoja o tiene un solo hijo
                return None
            elif self.getLeft() and self.getRight():  # Caso en el que el nodo tiene dos hijos
                successor = self.getRight().getMinimum()  # Encontrar el sucesor en orden en el subárbol derecho
                self.setRoot(successor.getRoot())  # Copiar el valor del sucesor al nodo actual
                self.setRight(self.getRight().delete(successor.getRoot()))  # Eliminar el sucesor del subárbol derecho
            else:  # Caso en el que el nodo tiene un hijo
                return self.getLeft() if self.getLeft() else self.getRight()
