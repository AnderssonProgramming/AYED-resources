...
class BinaryTree:
    ...
    def build_pos(self, lista_pre, lista_in, aux):
        if not lista_pre or not lista_in:
            return aux
        root = BinaryTree(lista_pre.pop(0))
        ind_root = lista_in.index(root.root)
        root.left = self.build_pos(lista_pre[:ind_root + 1], lista_in[:ind_root], aux)
        root.right = self.build_pos(lista_pre[ind_root:], lista_in[ind_root + 1:], aux)
        aux.append(root.root)
        return aux
...
def main():
    ...
    while linea:
        lista_pre, lista_in = map(list, linea.split())
        tree = BinaryTree(None)
        ans = tree.build_pos(lista_pre, lista_in, [])
        lista_pos = ""
        for index in ans:
            lista_pos += index
        print(lista_pos)
